const ContactListItem = ({ contact, onSelect, currentUser, isOnline }) => {
  const contactUser = contact.contactUser;

  // When clicking a contact, create a pseudo-chat object for the chat area
  const handleClick = () => {
    const pseudoChat = {
      _id: null,
  ...
  return (
    <div
      onClick={handleClick}
      className="flex items-center px-3 py-[10px] cursor-pointer transition-colors hover:bg-[#202c33]"
    >
      {/* Avatar */}
      <div className="relative mr-3">
        <div className="w-[49px] h-[49px] rounded-full bg-[#6b7b8d] flex-shrink-0 flex items-center justify-center overflow-hidden">
